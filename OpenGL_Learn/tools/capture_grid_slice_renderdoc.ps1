...
$analysisScript = Join-Path $PSScriptRoot "analyze_grid_slice_renderdoc.py"
...
$caseName = "cached-n0512-r060-s$($SliceCount.ToString('00'))"
$captureTemplate = Join-Path $captureDirectory $caseName
$capturePath = $captureTemplate + "_capture.rdc"
$diagnosticPath = Join-Path $diagnosticDirectory "$caseName.json"
$applicationImage = Join-Path $imageDirectory "$caseName-app.ppm"
$replayJson = Join-Path $replayDirectory "$caseName-replay.json"
...
$captureArguments = @(
    ...
    "--point-light-grid-slices", [string]$SliceCount,
...
Invoke-HiddenProcess -FilePath $renderDocCmd `
    -Arguments @("thumb", "--out", (Join-Path $imageDirectory "$caseName-thumbnail.png"), $capturePath) `
...
$env:GRID_SLICE_RDC = $capturePath
$env:GRID_SLICE_RDC_OUTPUT = $replayJson
$env:GRID_SLICE_DIAGNOSTIC = $diagnosticPath
$env:GRID_SLICE_EXPECTED = [string]$SliceCount
try {
    ...
    Remove-Item Env:GRID_SLICE_RDC -ErrorAction SilentlyContinue
    Remove-Item Env:GRID_SLICE_RDC_OUTPUT -ErrorAction SilentlyContinue
    Remove-Item Env:GRID_SLICE_DIAGNOSTIC -ErrorAction SilentlyContinue
    Remove-Item Env:GRID_SLICE_EXPECTED -ErrorAction SilentlyContinue
}
...
$manifest = [ordered]@{
    ...
    case = "cached N=512 R=6 S=$SliceCount"
...
Write-Host "RenderDoc grid-slice evidence: $evidenceDirectory"
