...
- 画面相关实验必须把同相机的 A/B 截图直接嵌入报告；不能只记录截图路径或仅在聊天中展示。
...
涉及渲染功能、shader、材质、纹理、光照、后处理或任何可能改变画面的实验，必须：

- 使用与性能 A/B 完全一致的场景、相机、分辨率和渲染设置保存 A/B 截图。
- 在项目报告中使用 Markdown 图片直接嵌入对比图，不能只写本地生成路径。
- 将报告使用的截图或对比图保存到可随报告提交的 `docs/benchmark-images/<task>/`；体积较大的原始捕获可以继续放在忽略目录。
- 对肉眼差异较弱的实验额外保存差异图或热力图，并说明差异是否集中在预期区域。
- 让自动 benchmark 在重跑时同步刷新报告图片，避免报告数据和截图来自不同实验批次。
- 在截图旁明确标注 A/B 含义，并记录人工视觉复核结论。

### 4.6 多档质量或算法模式

同一功能存在多个质量档位或算法阶段时，不能只比较“关闭”和最高档。
必须对相邻档位分别执行受控 A/B，例如
`off → hard → PCF → PCSS`，从而把基础功能成本、过滤成本和高级算法成本
分开记录。每一组都沿用相同的预热、进程数、交错顺序、场景和相机。

报告还必须记录决定工作量的专用参数，例如采样数、ring 数、搜索半径、
分辨率和 early-out 条件。若外部负载使进程范围出现反常或不收敛结果，
该批次必须标记为 rejected，保留数据和原因，再通过更长采样窗口或空闲
环境重测；不得从噪声批次中挑选有利样本。
...
- [ ] 画面相关实验的 A/B 截图已保存到项目并直接嵌入报告
- [ ] 肉眼差异较弱时已保存并嵌入差异图/热力图
